...
#iestatijumi
DATASET_DIR = "datu_kopa_trimmed"
OUTPUT_DIR = "dataset_landmarks_trimmed"
MODEL_PATH = os.path.join("models", "hand_landmarker.task")

#mapju nosaukumi mapītē dataset_trimmed
CLASS_FOLDERS = [
    "A",
    "AA_long_new",
    "plane_new",
...
def folder_to_label(folder_name: str) -> str:
    """
    Pārveido mapes nosaukumu uz klases label.
    Piemēram:
    AA_long_new -> AA_long
    plane_new   -> plane
    A           -> A
...
def process_video(video_path: str, landmarker):
    ...
    sequence = np.stack(sequence, axis=0)  #shape: (T, 21, 3)
